...
const (
	...
	// subscription to an object type from a specified counter value. Objects
	// with higher counter values will be sent to the client.
...
func (s *rpcServer) Stop() error {
	...
	for client := range s.clients {
		...
		if err != nil {
			state := rpcConstructState(client)
			rpcLog.Errorf("Problem closing client connection from %s: %v",
